import os
import dspy
from src.promtomatic.core.prompts import (
...
class DatasetConfig:
    """Configuration for different dataset types."""
    XSUM = {
...
def setup_config_logger():
    ...
    log_dir = Path("logs/config/llm_interactions")
...
class Config:
    ...
    def _populate_config_from_huggingface(self):
        ...
        self._develop_prompt_template_components()
    ...
    def _develop_prompt_template_components(self):
        """
        ...
        components = ['TASK', 'FEW_SHOT_EXAMPLES', 'CONTEXT', 'QUESTION']
        ...
        has_markers = any(f'[{comp}]' in self.raw_input_improvised for comp in components)
        
        if not has_markers:
            # If no markers found, treat entire input as task
            self.task = self.raw_input_improvised.strip()
            self.few_shot_examples = None
            self.task_context = None
            self.question = None
            return
        ...
        for comp in components:
            start = self.raw_input_improvised.find(f'[{comp}]')
            if start != -1:
        ...
        for i, (comp, pos) in enumerate(ordered_components):
            ...
            if i == len(ordered_components) - 1:
                content = self.raw_input_improvised[start:].strip()
            else:
                # Extract until the next component
                next_comp = ordered_components[i + 1][0]
                end = self.raw_input_improvised.find(f'[{next_comp}]')
                content = self.raw_input_improvised[start:end].strip()
    ...
    def _set_search_type_config(self):
        ...
        search_configs = {
            'quick_search': {
                ...
                'miprov2_compile_minibatch_size': 5
            },
            'moderate_search': {
                # DATA related
    ...
    def _extract_sample_data(self, tmp_lm) -> str:
        ...
        if self.few_shot_examples is not None:
            return convert_few_shot_examples_to_json(self.few_shot_examples)
        ...
        if self.task_context is not None or self.question is not None:
            ...
            complete_sample = tmp_lm(prompt)[0]
            if "```json" in complete_sample:
                complete_sample = complete_sample.split("```json")[1].strip()
            if "```" in complete_sample:
                complete_sample = complete_sample.split("```")[0].strip()
    ...
    def _extract_fields(self, tmp_lm) -> tuple[list, list]:
        ...
        try:
            allowed_fields = list(ast.literal_eval(self.sample_data)[0].keys())
        except Exception as e:
            logger.warning(f"Failed to parse fields JSON response: {str(e)}")

        if self.task_type == 'summarization' and 'summary' not in allowed_fields:
            allowed_fields.append('summary')
        elif self.task_type == 'classification' and 'label' not in allowed_fields:
            allowed_fields.append('label')
        elif self.task_type == 'generation' and 'target' not in allowed_fields:
            allowed_fields.append('target')
        elif (self.task_type == 'question-answering' or self.task_type == 'qa') and 'answer' not in allowed_fields:
            allowed_fields.append('answer')
        elif self.task_type == 'translation' and 'translation' not in allowed_fields:
            allowed_fields.append('translation')
        elif self.task_type == 'text-classification' and 'label' not in allowed_fields:
            allowed_fields.append('label')
        elif self.task_type == 'text-generation' and 'text' not in allowed_fields:
    ...
    def _process_human_feedback(self, tmp_lm) -> str:
        ...
        prompt = simplify_human_feedback(self.raw_input)
        response = tmp_lm(prompt)[0]
...
#     @classmethod
#     def from_provider(cls, provider: ModelProvider) -> 'APIConfig':
#         """Create API configuration based on the provider."""
#         config_map = {
